...
public class ClienteMB extends GenericoMB{
	...
	private boolean validar () {
		...
		if(razonSocial == null) {
			super.msgAlerta("Razón Social requerida");
			return false;
		}
		...
		if(razonSocial.length()<5 || razonSocial.length()>240) {
			super.msgAlerta("La Razón Social debe tener como minimo 5 caracteres y maximo 240");
			return false;
		}	
		...
		if(this.getCliente().getDireccion().trim().length()<10) {
			super.msgAlerta("La Dirección debe tener como mínimo 10 caracteres");
			return false;
		}	
